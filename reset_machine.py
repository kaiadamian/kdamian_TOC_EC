...
'''reset_machine.py - this program traces all the paths that a reset machine takes and outputs its information as well as its path to accept'''
                ...
                for trans in transitions[state]:            # go through each transition from current state
                    ...
                    if trans[1] == right_in[0]:             # if transition char matches the char we are reading, make the appropriate replacements and shifts
                        if trans[4] == 'R':
                            ...
                            left_in, right_in = go_reset(left_in, right_in, trans[3])
...
def main() -> None:
    '''main function'''
    file = "shift_left.csv"                       # set default parameters
    input = "bbaaaab"
    max_depth = 50
